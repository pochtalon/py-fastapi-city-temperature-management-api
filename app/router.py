...
from app.db import db_creating
router = APIRouter()
...
async def update_temperature(
        city_id: int,
        db: Session = Depends(db_creating.get_db)
) -> schemas.Temperature:
    ...
    api_url = (
        ...
        '{city.name}&appid=bb5878760b3f454b2ceaea8ed25f5021'
    )
        ...
        async with session.get(api_url) as response:
            if response.status != 200:
                ...
                raise HTTPException(
                    ...
                    detail=f"Error fetching weather data: {content}"
                )
